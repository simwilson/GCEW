 ...
 * \param[in] pull_mode Pin pull mode
 */
static inline void Settings_set_pull_mode(const enum port_pull_mode pull_mode)
{
	PORTD_set_pin_pull_mode(5, pull_mode);
}

/**
 * \brief Set Settings data direction
 *
 * Select if the pin data direction is input, output or disabled.
 * If disabled state is not possible, this function throws an assert.
 *
 ...
 *                      (low power state)
 */
static inline void Settings_set_dir(const enum port_dir dir)
{
	PORTD_set_pin_dir(5, dir);
}

/**
 * \brief Set Settings level
 *
 * Sets output level on a pin
 *
 * \param[in] level true  = Pin level set to "high" state
 *                  false = Pin level set to "low" state
 */
static inline void Settings_set_level(const bool level)
{
	PORTD_set_pin_level(5, level);
}

/**
 * \brief Toggle output level on Settings
 *
 * Toggle the pin level
 */
static inline void Settings_toggle_level()
{
	PORTD_toggle_pin_level(5);
}

/**
 * \brief Get level on Settings
 *
 * Reads the level on a pin
 */
static inline bool Settings_get_level()
{
